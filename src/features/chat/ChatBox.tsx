...
export const ChatBox = () => {
  ...
  const script = useMemo(
    () => scriptData(businessInfo, member.name || ''),
    [businessInfo],
  )
  ...
  const currentPlaceholder =
    script[stepIndex]?.placeholder ?? '메시지를 입력해주세요.'
  ...
  const pushUntilUserTurnWithDelay = async (
    baseDelay = 700,
    startAt?: number,
  ) => {
  ...
  const handleSelect = (
    ...
    setMessages((prev) => [
      ...prev,
      {
        stepId: current.id,
        role: 'user',
        contentText: selectedLabel,
        _key: `${performance.now()}-${stepIndex}`,
      },
    ])
    ...
    const target =
      typeof nextId === 'number' ? nextId : (current.nextId ?? stepIndex + 1)
    setStepIndex(target)
      ...
      <ChatBoxContainer ref={chatContainerRef}>
        {messages.map((m, idx) => {
          ...
          if (!step && m.contentNode) {
            return <div key={m._key}>{m.contentNode}</div>
          }
          if (step.role === 'bot' && step.input === 'select' && step.options) {
            ...
            return (
              <ChatBubbleItem
                key={m._key}
                message={renderContent(step, m.contentText)}
                isUser={m.role === 'user'}
                title={step.title}
                index={idx}
              />
