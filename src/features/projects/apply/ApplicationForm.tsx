  ...
  return (
    <PaddedBox style={{ gap: 28, display: 'flex', flexDirection: 'column' }}>
      ...
      <ApplicationTextArea
        label="컨설팅 신청 내용"
        index={15}
        value={values[BIZ_PREFIX + '컨설팅 신청 내용'] ?? ''}
        defaultValue={'폐업 과정에서 필수 신고 절차와 세무·채무 정리에 대한 이해가 부족하여 법적·행정적 리스크가 우려됩니다. 또한 집기·설비·재고 처분 방법에 대한 경험이 없어 처리 과정에서 손실이 예상되며, 건물주와의 철거 범위 협의 및 분쟁 가능성도 부담으로 작용하고 있습니다. 임대차 계약 관련 문제와 향후 신용 회복 방안까지 전문적인 조언이 필요한 상황이므로, 폐업 전반에 걸친 종합적 컨설팅 지원을 받고자 합니다.'}
        onChange={(v) =>
      ...
      <ApplicationTextArea
        label="원상복구 신청 내용"    
        index={16}
        value={values[BIZ_PREFIX + '원상복구 신청 내용'] ?? ''}
        defaultValue={'임대차 계약 종료에 따라 점포 내 집기·설비 철거 및 원상복구가 필요하나, 철거 범위가 넓고 소요 비용이 상당하여 경제적 부담이 큽니다. 특히 면적이 넓어 철거비용이 50만 원을 초과할 것으로 예상되며, 자력으로는 비용 충당이 어려운 상황입니다. 이에 사업정리 도우미 지원사업을 통해 전용면적 기준에 따른 원상복구 비용 지원을 신청하며, 신청 후 3개월 내 철거 완료 조건을 충족해 성실히 이행할 계획입니다.'}
        onChange={(v) =>
